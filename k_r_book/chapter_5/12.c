...
int parse_c_m(int argc, char *argv[], int *arguments);
int parse_t(int argc, char *argv[], int *arguments);
...
int main(int argc, char *argv[]) {
    ...
    if (argument_count >= 0) {
        tabstops = malloc(argument_count * sizeof(int));
...
int parse_tabstops(int argc, char *argv[], int *arguments) {
    ...
    if (**argv == '-' && *(*argv + 1) == 't') {
        return parse_t(argc, argv, arguments); 
    } else if (**argv == '-' && 
            atoi(*argv + 1) != 0 &&
            **(argv + 1) == '+' &&
            atoi(*(argv + 1) + 1) != 0) {
        return parse_c_m(argc, argv, arguments);
    } else {
